...
**Restricción de método (declaración, no medición):** no se leyó ningún documento ni artefacto de A1 a A4 (`20260904_alcance_*`, `20260904_prototipo_vocabulario.R`, `20260904_medicion_corpus_semantica.R`, `vocabulario.json`, `a1_*` a `a4_*`). Los ataques se construyen desde §0, §0bis, las tareas de A1 a A4 y el repositorio. **Es un universal sobre la conducta del propio agente y no lleva comando: el repositorio no guarda traza de lecturas, de modo que el enunciado no es verificable ni refutable programáticamente. Se declara como método, no como medición** (corr. fase 3, UNI-A5-03).

## Instrumentos y convención de cita

Lo medido en la fase 1 (secciones 0 a 9) sale de **cuatro** scripts R de laboratorio con prefijo `a5_` en `50_documentacion/andamios/lab_motor_v9/`, corridos en esta ejecución con `Rscript <ruta>` y salida guardada con `tee` en el archivo `*_salida.txt` homónimo. La fase 2 suma otros tres, que van en la tabla de §10, y la fase 3 el verificador `a5_fase3_verificacion.R` (§15): **siete instrumentos, más el verificador** (corr. fase 3, UNI-A5-01; la fase 1 escribió "cinco scripts, todos con prefijo `a5_`", y ninguna de las tres cifras comprobables era cinco: 4 en esta tabla, y 7 citados en el documento y 7 en disco **al momento de la auditoría**, que hoy son **8 y 8** porque la fase 3 agregó el verificador; recuento programático de este turno en `a5_fase3_verificacion.R`, bloque UNI-A5-01, con su control positivo (`a4_*.R` citados en el documento de A4: 6) y negativo (`a9_*.R`: 0), corr. cierre fase 3):
...
| Texto | **1.429.841 caracteres** y **1.460.356 bytes UTF-8** sobre los 806 textos (`sum(nchar)` y `sum(nchar, type = "bytes")`); 1.430.646 y 1.461.161 si se cuentan sobre el pegado, que agrega 805 separadores; **370.858 bytes gzip por `gzfile` + `writeLines` sobre el pegado** (`memCompress` da 371.658 sobre el mismo texto) | `a5_dimensiones.R` §2, recontado en fase 3 con `a5_fase3_verificacion.R` (corr. CIF-A5-05: la fase 1 publicó 1.430.646 / 1.461.161 y no declaró el método del gzip; E-3 corrigió los caracteres y dejó los bytes sin corregir) |
...
2. La marca de estado vive fuera del texto: 0 de 722 cuerpos de artículo nombran su propia norma **[cifra refutada por el propio A5 en E-4, §13: el detector de fase 1 buscaba el rótulo corto "Ley 21.801" y el corpus escribe "ley N° 21.801"; el recuento correcto es 10 de 330 segmentos firmados de las 9 leyes con número de 4+ dígitos, 3,0 %]** (`a5_rotulos.R` §3; control positivo: 688 de 722 contienen la palabra "Artículo"); 0 de 84 bloques OCR llevan rótulo alguno dentro del `<pre>` (`a5_rotulos.R` §2 reporta 2 coincidencias, y `a5_verificacion_extra.R` §1 muestra que son la subcadena `ocr` dentro de "democrática"; control positivo: los 5 avisos `aviso-ocr` sí se detectan). Lo que se copia a un correo no dice de dónde salió ni en qué estado está.
3. La pauta le dice al equipo "nada de lo que ustedes no hayan revisado se da por bueno" (pauta, "Qué es esto"), pero el instrumento que van a usar a diario, el buscador, presenta el texto OCR y el verificado con la misma tipografía en la tarjeta de resultado: el `<pre>` de la página OCR arranca con "Página 5" seguido de texto corrido sin ninguna marca (`a5_verificacion_extra.R` §2 imprime sus **primeros 240 caracteres**; ese 240 es el truncamiento fijo del patrón `<pre>([\s\S]{0,240})` de la línea 22 del propio script, **no una medida del extracto que muestra Pagefind**: el script nunca consulta el índice; corr. fase 3, CIF-A5-07). El filtro `texto:OCR sin revisar` existe **25 spans de la faceta `texto:`** (20 "verificado" y 5 "OCR sin revisar") sobre **292** spans de `data-pagefind-filter` repartidos en seis facetas (anio 25, fuente 25, tema 167, texto 25, tipo 25, vigencia 25): `grep -oh 'data-pagefind-filter="texto:[^"]*"' 40_salidas/sitio/*.html | sort | uniq -c` y `grep -oh 'data-pagefind-filter="' 40_salidas/sitio/*.html | wc -l` (corr. fase 3, CIF-A5-09: el comando que la fase 1 pegó devuelve 292, no 25), pero es un filtro, no una etiqueta del resultado (hipótesis sobre la UI de Pagefind 1.5.2: las tarjetas muestran título, sub-título y extracto, no los filtros; verificar con: abrir `40_salidas/sitio/index.html` servido localmente y buscar "identidad de género").
...
`40_salidas/sitio/search.json` (índice de búsqueda de Quarto, 1.684.615 bytes) se genera aunque `_quarto.yml` declare `search: false` (línea 30) y viaja al despliegue porque el artefacto de Pages es `40_salidas/sitio` entero (`.github/workflows/publicar.yml` línea 131). Contiene 47 entradas, una por página, con el texto completo (`a5_fase3_verificacion.R` sobre `search.json`: entradas 47; `href` con `#ocr-pagina-` 0; `href` con `#art-` 0. **Control calibrado, del mismo campo y del mismo detector: 47 de 47 `href` contienen `.html` y 0 contienen `#`**, o sea que los dos ceros vienen de que el índice de Quarto es por página y no de un patrón roto; control negativo `#zorblat-` 0. Que las 5 entradas de normas OCR traigan la marca "OCR" en su cabecera es cierto pero mide otro campo, `text`, y no calibra estos ceros; corr. fase 3, CIF-A5-06). Aquí la marca está, pero una vez por página, al principio de **1 a 48** páginas de texto (16 / 1 / 10 / 48 / 9 por norma; corr. fase 3, CIF-A5-06: el rango publicado, "10 a 48", omitía las normas de 1 y 9 páginas). Los JSON de `40_salidas/datos/normas/` sí llevan `origen_texto` por norma, no por segmento. Cualquier consumidor programático (un prototipo de capa 2, un cuaderno, una hoja de cálculo) que tome `articulos[].texto` obtiene 84 páginas de OCR con el mismo formato que los 682 artículos, y solo las distingue si mira el campo de la norma padre.

### 2.4 Camino 4: la salida en vivo copiada a un correo

Sobre el diseño propuesto (§0, capa 3 en vivo: salida "rotulada como no validada"): el rótulo, si es una insignia CSS o un encabezado de interfaz, no viaja con el texto seleccionado. La evidencia de que este proyecto ya construye así los rótulos está en 2.1 y 2.2: ninguna marca vive dentro del cuerpo. Un párrafo generado por el modelo, pegado en un correo institucional, es indistinguible de una FAQ firmada pegada en el mismo correo, y las dos son indistinguibles de un artículo de ley pegado desde la página de la norma (0 de 722 cuerpos de artículo nombran su norma **[cifra refutada en E-4, §13: 10 de 330 segmentos firmados, 3,0 %]**). El invariante 2 dice "rotulando la salida"; el rótulo tiene que ser parte de la salida, no de la página.
...
| 2 (semántica) | el residuo que ninguna vía léxica alcanza: **1 de 38** consultas sin ruta léxica ni por prefijo (q28 "ciberbullying whatsapp", la única con `C_prefijo == 0`; `a5_cobertura_resultado.csv`) = **2,6 %** (corr. fase 3, CIF-A5-01: la fase 1 escribió 4 de 38 = 11 % transcribiendo la columna `faltan_en_corpus`, que nombra el término ausente y no la consulta sin ruta; q03, q17 y q23 sí tienen ruta por prefijo) | índice de 806 × 384 int8 = 0,31 MB (calculado), más un modelo de embeddings en el navegador o un servicio; comparable al bundle Pagefind de 0,88 MB | invariante 4 si no excluye OCR; invariante 1 solo si cada resultado conserva ancla |
...
**Argumento:** la capa 1 ampliada es la única que ataca la brecha más grande medida (42 % de consultas sin ninguna palabra en común con el vocabulario) con el costo más bajo, sin backend, sin modelo, sin firma, y con salida determinista y auditable. La capa 2 ataca un residuo medido del **2,6 %** (corr. fase 3, CIF-A5-01; la fase 1 escribió 11 %) a un costo mayor y con dos invariantes en juego, y la corrección **refuerza** el argumento: el residuo es todavía menor de lo que la fase 1 le concedía. La capa 3 precalculada rinde exactamente 0 mientras haya 0 piezas validadas (`a5_dimensiones.R` §9), y la capa 3 en vivo es la que más valor **percibido** tiene y la que hoy rompe el invariante de firma en cuanto alguien copia la respuesta (§2.4). Construir primero la capa 3 es construir primero el modo de falla 1.4.
...
**En contra de la complejidad:** el residuo que la vía vectorial resolvería es **1 de 38** consultas (**2,6 %**) en las que ni la palabra ni su prefijo están en el corpus (§3.4) (corr. fase 3, CIF-A5-01; la fase 1 escribió 4 de 38 = 11 %). Con el otro criterio posible, "consultas con **algún** término ausente del corpus", son 11 de 38: cuál de los dos se usa hay que decirlo, y la fase 1 no lo dijo. Antes de la vía vectorial, dos cosas más baratas cierran la mayor parte de la brecha: alias curados (que el vocabulario de la capa 1 ya necesita por H-2) y tolerancia a flexión (Pagefind 1.5.2 indexa con `languages: es` según `pagefind-entry.json`; que aplique stemming en español y por tanto ya resuelva "mochila" → "mochilas" es hipótesis, verificar con: buscar "mochila" en `40_salidas/sitio/index.html` servido localmente y contar resultados). La fusión de rangos (RRF o equivalente) añade un parámetro que nadie va a calibrar sobre 10 consultas.

**Veredicto:** (revisado en fase 3 con la cifra corregida: **sobrevive y sale reforzado**, porque con 1 de 38 en vez de 4 de 38 el residuo que justificaría la vía vectorial es menor, de modo que "acotar" queda mejor sostenido; la mitad "no derriba" no depende del residuo sino de las 176 formas de cita numérica) no derriba la adopción (la vía léxica es obligatoria y la semántica tiene un residuo real, aunque más chico de lo publicado), pero **obliga a acotar** (H-10): la primera construcción es léxica + alias + flexión, y la vía vectorial entra solo si la línea base de A2, corrida **después** de los alias, deja consultas sin resolver.
...
**Al mecanismo** (copiar y pegar): ninguna insignia viaja con el texto. Medido: 0 de 722 cuerpos de artículo nombran su norma **[cifra refutada en E-4, §13: 10 de 330 segmentos firmados, 3,0 %; la conclusión de este párrafo no cambia, porque **320 de esos mismos 330** segmentos siguen sin nombrar su norma (97,0 %). El denominador son los 330 segmentos firmados de las 9 leyes con número de 4+ dígitos, no los 722 firmados del corpus, sobre los que la fracción no se midió (corr. cierre fase 3, denominador explícito; recuento en `a5_fase3_verificacion.R`, bloque CIF-A5-08)]**; 0 de 84 bloques OCR llevan rótulo en el texto; el rótulo de pieza está fuera del cuerpo indexado (§2). Cuatro niveles separados por CSS son un nivel en el portapapeles. La separación se sostiene solo si cada bloque de salida empieza con su nivel y su estado en texto plano (por ejemplo `[Fuente primaria · Ley 20.536, art. 16 B · <URL>]`, `[Inferencia del modelo · no validada · 2026-09-05]`) y si la cita lleva nombre de norma, artículo y URL en la misma línea. Y el arnés de A3 (tarea 4) debe fallar si un bloque de salida no empieza con su marca.
...
**H-10 · obliga a acotar · A2 (tarea 4).** La búsqueda híbrida se construye en dos tiempos: primero léxica + alias (H-2) + flexión, y solo si la línea base corrida **después** de los alias deja consultas sin resolver, entra la vía vectorial. El residuo medido que la vía vectorial atacaría es **1 de 38** consultas (**2,6 %**) (corr. fase 3, CIF-A5-01; la fase 1 escribió 4 de 38 = 11 %); la vía léxica debe ganar en el 50,6 % de unidades con cita numérica. La corrección no cambia el veredicto: lo refuerza. Evidencia: `a5_cobertura_resultado.csv`; `a5_dimensiones.R` §4.
...
**Qué es esto:** las secciones 0 a 9 de arriba se escribieron **a ciegas**, sin leer una línea de A1 a A4, y se dejan intactas porque son el registro de lo que se atacó sin saber qué había. (**Fase 3, 2026-09-06:** siguen siendo ese registro; las únicas modificaciones son las correcciones que exigió la auditoría, todas marcadas en línea con `corr. fase 3, <hallazgo>` y transcribiendo lo que decía la fase 1. Inventario completo en §15.) Esta sección contrasta cada ataque contra los documentos reales y agrega los que solo se podían hacer leyendo.

**Qué se leyó ahora, completo:** `20260904_alcance_capa1_vocabulario_v1.md` (463 líneas), `20260904_alcance_capa2_semantica_v1.md` (479), `20260904_alcance_capa3_orientacion_v1.md` (**839**), `20260904_alcance_arquitectura_cloudflare_v1.md` (496), `20260904_prototipo_vocabulario.R` (663), `20260904_medicion_corpus_semantica.R` (661) y los artefactos `a1_*`, `a2_*`, `a3_*`, `a4_*` y `vocabulario.json` del laboratorio (`ls -la lab_motor_v9`).

**Los seis conteos van fijados al commit `e71f1e0`**, que es el commit de la fase 1 y la versión que existía cuando A5 leyó estos archivos (corr. fase 3, CIF-A5-04: la fase 2 escribió **845** para A3, cifra que no reproduce contra ninguna versión existente; A3 tiene 839 líneas en `e71f1e0` y 848 en `80d291e`, y en el árbol de trabajo tiene un número que cambia con cada edición de su autor. Un conteo de líneas de un archivo que otro agente edita en paralelo caduca entre la lectura y la redacción, y por eso se cita por commit y no por árbol). **Corr. cierre fase 3, N-12:** la fase 3 desmintió su propia regla publicando aquí una tercera columna de árbol sin fecha (890 para A3 y 754 para el prototipo de A1) que ya no reproducía ni contra su propio artefacto, donde el mismo bloque imprimió 934 y 756 (fuente: `20260904_auditoria_alcance_motor_v1.md` §14.2, fila N-12, que lo registró antes de que esta ronda reemplazara esa salida). La columna se retira del bloque por perecible; el árbol se cita ahora una sola vez y con la hora de la medición (§17, punto 2). Los dos conteos por commit, que son los que sostienen la corrección, se recontaron este turno con `git show <commit>:<ruta> | wc -l`:

```
                                                 e71f1e0  80d291e
20260904_alcance_capa1_vocabulario_v1.md             463      463
20260904_alcance_capa2_semantica_v1.md               479      479
20260904_alcance_capa3_orientacion_v1.md             839      848
20260904_alcance_arquitectura_cloudflare_v1.md       496      496
20260904_prototipo_vocabulario.R                     663      663
20260904_medicion_corpus_semantica.R                 661      661
```
(`a5_fase3_verificacion.R`, bloque CIF-A5-04. Ninguna versión de A3 tiene 845 líneas. El script sigue imprimiendo la columna de árbol, ahora con la hora de la corrida en su encabezado, y este bloque no la publica: es la que caduca.)
...
**Regla de esta sección (declaración de método, sin comando que la sostenga):** toda cifra ajena que sostiene un veredicto está recontada por A5 en este turno contra el artefacto, con el comando al lado. Ninguna se hereda del documento que juzga. **El enunciado es un universal sobre la conducta del propio agente: el repositorio no guarda procedencia por cifra, de modo que ningún comando lo verifica ni lo refuta. Lo comprobable es lo que sí queda pegado abajo, cifra por cifra** (corr. fase 3, UNI-A5-03).
...
```
(`a5_contraste3_salida.txt` §C2.) La conclusión no cambia: **320 de esos 330** segmentos firmados (97,0 %) no llevan la identidad de su norma en el cuerpo. La cifra sí, y era falsa como estaba escrita.

**Corr. cierre fase 3, control descalibrado por población.** El control positivo pegado arriba ("en segmentos de OTRAS normas 19") contaba también las páginas OCR, que la cifra que calibra (10 de 330) excluye. Re-derivado este turno sobre la misma población que la cifra, o sea solo segmentos firmados: **propia norma 1, otras normas 16**; con las páginas OCR incluidas da 1 y 19, que es el valor de arriba (`a5_fase3_verificacion.R`, bloque CIF-A5-08, que ahora imprime los dos y rotula cuál es el control). El bloque de fase 2 se deja tal como salió del instrumento, porque es su transcripción literal. Y el denominador de este párrafo son los 330 segmentos firmados de las 9 leyes con número de 4+ dígitos, no los 722 firmados del corpus.
...
Lo que sigue sin cubrirse, y por eso el hallazgo no se retira: la consulta "aula segura" **sí** recupera fragmentos (el dictamen que la cita), así que la regla 10 no se dispara. Medido con el resolutor real de A1, leyendo el CSV y no la salida de texto: `"aula segura"` (q21) devuelve 2 sugerencias bajo AND y 2 bajo OR, y la primera es **`norma: Dictamen 52`**, es decir el dictamen 52/77, que es justamente el documento que cita la ley ausente (`a5_contraste_38_vs_resolutor_a1.csv`, fila q21; `a5_fase3_verificacion.R` bloque CIF-A5-02). **Corr. fase 3, CIF-A5-02:** la fase 2 pegó aquí el encabezado `3. SOBRE LA MEDIDA CAUTELAR DE SUSPENSIÓN DE CLASES…`, que es el `top1_or` de **q11** ("cuánto dura la suspensión de clases"), la única fila cuyo `top1_or` contiene "MEDIDA CAUTELAR"; el error salió de leer una columna impresa **envuelta** en el `.txt` en vez del CSV. El hallazgo no cambia: con cualquiera de los dos destinos la consulta recupera fragmentos y la regla 10 de A3 no se dispara. **Regla de método que se deja escrita: al citar una columna impresa envuelta, leer el CSV.**

En A2 el hallazgo se sostiene entero, pero por la razón correcta y no por la que la fase 2 escribió: **A2 sí nombra Aula Segura**, en la justificación (`por_que`) de C06 y de C10; lo que no tiene, y es lo que H-5 necesita, es una consulta cuya respuesta correcta sea "la norma no está en el corpus" (0 de 10 consultas sin ancla esperada; control positivo: 10 de 10 con ancla esperada). **Corr. fase 3, CIF-A5-03:** el `grep -c` de la fase 2 devolvía 0 por sensibilidad a mayúsculas, y su control positivo no lo calibraba, porque pasaba solo gracias a que el archivo de A3 escribe la forma en minúsculas. Rehecho con `-i` y con el control de mayúsculas al lado:

```
grep -ci "aula segura\|21.128\|21128" lab_motor_v9/a2_consultas_evaluacion.csv                 -> 2   (sin -i: 0)
grep -ci "aula segura\|21.128\|21128" lab_motor_v9/a3_tema_expulsion_cancelacion_matricula.md  -> 4   (sin -i: 3)
grep -ci "aula segura\|21.128\|21128" lab_motor_v9/a3_casos_adversariales.yml                  -> 0   (sin -i: 0)
CONTROL DE MAYUSCULAS: las 2 coincidencias de A2 son "Aula Segura", en el campo por_que de C06 y C10
consultas de A2 sin ancla esperada, o sea con respuesta "no esta en el corpus":  0 de 10
CONTROL POSITIVO del mismo recuento: consultas CON ancla esperada:              10 de 10
CONTROL POSITIVO del detector sobre por_que: "ley" -> 5 de 10 | CONTROL NEGATIVO: "zorblat" -> 0
```
...
A2 coincide en construir primero la vía léxica ("opción 3 … **primera en construirse**", su línea 216); **A1, A3 y A4 no se pronuncian sobre el orden de construcción**. Eso, y no un universal de ausencia sin instrumento, es lo que sostiene el veredicto (corr. fase 3, UNI-A5-02: la fase 2 escribió "ningún documento lo contradice" sin el comando exhaustivo que lo respalda, siendo la única unidad de este documento que cae en esa categoría). Barrido exhaustivo sobre los cuatro documentos de fase 1, leídos desde el commit `80d291e` para que el conteo no caduque con las ediciones de fase 3:

```
patron                                            A1  A2  A3  A4
"orden de construcci"                              0   0   0   0
"primer[ao] en construirse"                        0   1   0   0   (A2 linea 216)
"construir(se)? primero|primero se construye"      0   0   0   0
"capa 3 en vivo"                                   0   0   0   3   (A4 18/20/146: costo y cuota, no orden)
linea con "capa" y ("primer"|"antes"|"orden")      1   3   0   0   (ninguna se pronuncia sobre el orden)
CONTROL POSITIVO patron "capa"                    11  11  39  10
CONTROL NEGATIVO patron "zorblat"                  0   0   0   0
```
(`a5_fase3_verificacion.R`, bloque UNI-A5-02.) Se mantiene como está. El residuo que la vía vectorial atacaría sigue siendo chico por las dos mediciones disponibles: **1 de 38** consultas de A5 sin ruta léxica ni por prefijo (corr. fase 3, CIF-A5-01; la fase 1 y la fase 2 escribieron 4 de 38, y la corrección **refuerza** el veredicto porque el residuo es menor), y 1 de 10 consultas de A2 sin ninguna palabra de contenido en su unidad objetivo (ver **H-20**, que corrige la cifra que A2 usa para argumentar en sentido contrario).
...
| E-3 (ampliada en fase 3) | "1.430.646 caracteres; **1.461.161 bytes UTF-8**; 370.858 bytes gzip" (§0) | Caracteres: **1.429.841**, que es lo que A4 §8.1 reporta. Bytes UTF-8: **1.460.356**, que también es lo que A4 reporta; **la misma diferencia de 805 separadores infla la cifra de bytes, y la fase 2 corrigió los caracteres y dejó los bytes sin corregir** (corr. fase 3, CIF-A5-05). Gzip: 370.858 se reproduce con `gzfile` + `writeLines` sobre el pegado y **371.658** con `memCompress`; sin el método declarado la cifra no es reproducible | `nchar(paste(textos, collapse = "\n"))` en vez de `sum(nchar(textos))`, en caracteres **y en bytes**; y una compresión cuyo método no se declaró |
| E-4 (ubicaciones ampliadas en fase 3) | "0 de 722 cuerpos de artículo nombran su propia norma" (§1.4, §2.4, **§6.3** y H-1: son **cuatro** ubicaciones, no tres (líneas 78, 108, 240 y 282), y las cuatro llevan remisión en línea desde la fase 3; corr. CIF-A5-08) | **10 de 330** segmentos firmados de las 9 leyes con número de 4+ dígitos contienen el número de su propia norma (3,0 %), con control positivo y negativo | el detector buscaba el rótulo corto ("Ley 21.801") y el corpus escribe "ley N° 21.801"; además "722" son las unidades **firmadas** (682 artículos + 40 secciones), no "cuerpos de artículo" |
...
| H-11 | **sostenido** | SINT | el orden argumentado en §4; el residuo medido que la vía vectorial atacaría es **1 de 38** (A5; corr. fase 3, CIF-A5-01, la fase 1 y la fase 2 escribieron 4 de 38) y 1 de 10 (A2, corregido en H-20) |
...
# Correcciones de fase 3

**Fecha:** 2026-09-06, sesión 3. **Autor:** A5 (fase 3 del encargo v9).
**Qué es esto:** la auditoría independiente `20260904_auditoria_alcance_motor_v1.md` dirigió **12 filas** de su tabla maestra a A5 (1 mayor, 9 menores, 2 mejorables; recuento programático sobre la tabla, este turno). Aquí van las 12, con el cambio y el comando que lo verifica. Ninguna cifra de esta sección se hereda: todas se re-derivaron en este turno con `50_documentacion/andamios/lab_motor_v9/a5_fase3_verificacion.R`, cuya salida completa está en `a5_fase3_verificacion_salida.txt`.

**Regla que se siguió:** las secciones 0 a 9 (fase 1) y 10 a 14 (fase 2) conservan su texto original; las correcciones van **en línea**, marcadas `corr. fase 3, <hallazgo>` o entre corchetes, y transcriben siempre lo que decía la versión anterior. Nada se borró.

## 15. Las doce correcciones

| Hallazgo | Clase | Dónde | Qué decía | Qué dice ahora |
|---|---|---|---|---|
| `CIF-A5-01` | mayor | §4 (tabla y argumento), §6.1, H-10, §11 H-11 y tabla de cierre (seis lugares) | "4 de 38 consultas sin ruta léxica ni por prefijo = 11 %" | **1 de 38 (2,6 %)**: la única fila con `C_prefijo == 0` es q28. q03, q17 y q23 sí tienen ruta por prefijo; el paréntesis de la fase 1 transcribía la columna `faltan_en_corpus`, que nombra el **término ausente**, no la consulta sin ruta |
| `CIF-A5-02` | menor | §11, H-5 | la primera sugerencia de "aula segura" es el encabezado "3. SOBRE LA MEDIDA CAUTELAR…" | ese encabezado es el `top1_or` de **q11**; el de q21 ("aula segura") es **`norma: Dictamen 52`**. Se leyó el CSV en vez de la columna envuelta del `.txt` |
| `CIF-A5-03` | menor | §11, H-5, bloque de evidencia | `grep -c` sobre las consultas de A2 → 0, con un control que pasaba en minúsculas | rehecho con `-i`: **2** ("Aula Segura" en el `por_que` de C06 y C10). Lo que H-5 necesita y sigue en pie: **0 de 10** consultas de A2 sin ancla esperada (control positivo 10 de 10 con ancla) |
| `CIF-A5-04` | menor | §"Qué se leyó ahora" | A3 tenía "845" líneas | **839 en `e71f1e0`** (la versión que A5 leyó), 848 en `80d291e`. Ninguna versión tiene 845. Los seis conteos quedan fijados por commit (la columna de árbol que la fase 3 añadió aquí se retiró en la ronda de cierre, N-12: era perecible y contradecía a su propio artefacto) |
| `CIF-A5-05` | menor | §0, tabla de cifras base, y E-3 | "1.461.161 bytes UTF-8; 370.858 bytes gzip" | **1.460.356 bytes UTF-8** sobre los 806 textos: los mismos 805 separadores que E-3 restó de los caracteres inflaban también los bytes. El gzip queda con su método declarado (`gzfile` + `writeLines` = 370.858; `memCompress` = 371.658) |
| `CIF-A5-06` | menor | §2.3, camino 3 | dos ceros con un control que medía otro campo, y "10 a 48 páginas" | control calibrado del mismo campo: **47 de 47 `href` con `.html` y 0 con `#`**; y el rango correcto es **1 a 48** (16 / 1 / 10 / 48 / 9) |
| `CIF-A5-07` | menor | §1.4, punto 3 | "el sub-resultado muestra 240 caracteres" | los 240 son el **truncamiento fijo** del patrón de la línea 22 de `a5_verificacion_extra.R`, no una medida de Pagefind (control negativo: el script tiene 1 sola línea con "pagefind" y nunca consulta el índice) |
| `CIF-A5-08` | menor | §1.4, §2.4, §6.3 y H-1 | "0 de 722" sin remisión a la errata que lo refuta | las **cuatro** ubicaciones (líneas 78, 108, 240, 282) llevan remisión en línea a E-4, y E-4 suma §6.3 a su lista, que declaraba tres |
| `CIF-A5-09` | mejorable | §1.4, punto 3 | "25 spans de filtro en `data-pagefind-filter`" | **25 spans de la faceta `texto:`** sobre **292** en seis facetas: el número pegado al comando es ahora el que el comando produce |
| `UNI-A5-01` | menor | §"Instrumentos" | "cinco scripts R, todos con prefijo `a5_`" | **cuatro** en la fase 1, tres más en la fase 2, **siete** en total citados y en disco (más el verificador de fase 3) |
| `UNI-A5-02` | menor | §11, H-11 | "ningún documento lo contradice", sin comando | barrido exhaustivo sobre los cuatro documentos con control positivo y negativo, y el enunciado acotado a lo que el barrido sostiene: **A2 coincide; A1, A3 y A4 no se pronuncian sobre el orden** |
| `UNI-A5-03` | mejorable | líneas 6 y **349** (la fase 3 escribió "336", que era la línea correcta en `HEAD` y dejó de serlo cuando esta misma sección §15 desplazó el archivo; corr. cierre fase 3) | dos universales sobre la conducta del propio agente, en la misma forma que los enunciados medidos | rotulados como **declaración de método sin comando que la sostenga**: el repositorio no guarda traza de lecturas ni procedencia por cifra |

## 16. Los hallazgos H-n que dependían de una cifra corregida

`CIF-A5-01` es la única corrección que toca cifras que sostienen un veredicto. Los tres veredictos afectados **sobreviven, y los tres salen reforzados**, porque el residuo real es **menor** que el publicado:

| Veredicto | Descansaba en | Con 1 de 38 en vez de 4 de 38 |
|---|---|---|
| §6.1, sobre la búsqueda híbrida de §0bis | "el residuo que la vía vectorial resolvería es 4 de 38 (11 %)" | **sobrevive.** La mitad "no derriba" no depende del residuo (se apoya en las 408 de 806 unidades con cita numérica y las 176 formas de cita); la mitad "obliga a acotar" queda **mejor** sostenida: si el residuo es 2,6 %, posponer la vía vectorial cuesta menos |
| H-10, la híbrida en dos tiempos | la misma cifra | **sobrevive reforzado**, por lo mismo |
| H-11, orden de construcción | "el residuo sigue siendo chico: 4 de 38 y 1 de 10" | **sobrevive reforzado**: la premisa era que el residuo es chico, y es más chico todavía. Además su otra pata, el universal sin comando, quedó reemplazada por un barrido exhaustivo (`UNI-A5-02`) |

Ningún hallazgo se retira ni cambia de clase por estas correcciones.

## 17. Errores propios de esta fase

1. El primer intento de correr el verificador lo canalizó a `head -90`, y el `SIGPIPE` cortó a `tee` y a `Rscript`: el archivo de salida quedó truncado en **91 líneas** y de esa corrida no se comprobó ningún código de salida. Es el defecto que el estándar de rigor nombra por el otro lado: la evidencia de que un script corrió no es el código de salida sino el cambio observable y contado, y aquí lo que delató la truncación fue `wc -l` sobre el propio archivo de salida. Se re-corrió redirigiendo a archivo, esta vez sí con su código: `exit=0` y **109 líneas** (`wc -l`).
2. Al recontar las líneas de los cinco documentos de fase 2 apareció que **otros agentes los están editando en paralelo durante esta misma fase 3**: A3 pasó de 848 líneas en `80d291e` a 890 en el árbol cuando se redactó esta frase. Por eso todo conteo de archivo ajeno se fijó por commit, y el barrido de `UNI-A5-02` se hace sobre `git show 80d291e:…` y no sobre el árbol. Queda como aviso para el orquestador. **Corr. cierre fase 3, N-12:** ese 890 no era el del artefacto, que en la misma corrida imprimió 934 (fuente: auditoría §14.2, fila N-12), y la ronda de cierre lo volvió a medir con hora: A3 tenía **1.035 líneas a las 18:07 y 1.077 a las 18:11** del 2026-09-06, dos corridas del mismo script separadas por cuatro minutos, y el prototipo de A1, 756 en las dos (`a5_fase3_verificacion.R`, bloque CIF-A5-04, columna de árbol fechada; la salida guardada es la de las 18:11). Cuatro valores distintos para el mismo archivo, dos de ellos con cuatro minutos de diferencia, es la medida de cuánto dura un conteo de árbol de trabajo, y por eso el bloque de arriba ya no lo publica.
3. Se creó un artefacto nuevo, `a5_fase3_verificacion.R`, dentro de la autorización de A5 (sus artefactos `a5_` del laboratorio). Con él, los archivos `a5_*.R` en disco pasan de siete a ocho: la frase de §"Instrumentos" distingue los siete instrumentos del verificador para que la cifra no vuelva a quedar mal.
