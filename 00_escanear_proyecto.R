...
#
# v4: instala la guarda de locale (POLITICA §5.2bis). Este script es un punto
#     de entrada propio: no pasa por 10_utils/10_configuracion.R y sin embargo
#     escribe texto acentuado ("Raíz", "Fecha", "CONTEO POR EXTENSIÓN", los
#     glifos del árbol). Corrido desde un shell sin locale escribía el snapshot
#     con todo eso escapado, sin error visible, que es exactamente la condición
#     que §5.2bis prohíbe.
# =============================================================================
...
# Va aquí y no más abajo: después del anclaje, porque necesita ROOT, y antes de
# cualquier lectura o escritura, porque un bootstrap posterior protegería los
# literales de este archivo pero no los usos anteriores a él.
# Se usa file.path(ROOT, ...) y no here::here() a propósito: este script puede
# invocarse con el working directory fuera del proyecto, y ROOT ya está anclado
# por rprojroot dos líneas más arriba.
ruta_guarda <- file.path(ROOT, "10_utils", "10_locale.R")
...
if (!file.exists(ruta_guarda)) {
  stop(
    ...
    "  acentuado escapado como <c3><a1> (Raíz, Fecha, los glifos del árbol),\n",
    "  sin error visible.\n",
    "  Remedio: copiar herramientas_dev/plantillas/10_locale.R a\n",
    "  10_utils/10_locale.R e invocar asegurar_locale_utf8() como primera\n",
    "  línea ejecutable de 10_utils/10_configuracion.R (POLITICA §5.2bis).",
    call. = FALSE
  )
}
...
# ---- Configuración ----------------------------------------------------------
# POLITICA §7.2: node_modules/, packrat/ y venv/ se excluyen junto con las de
# siempre. Incluirlos infla el conteo hasta volverlo inútil (evidencia medida:
# 461 archivos declarados donde slep_aprendizajes_ep tiene 289).
EXCLUIR <- c(".git", "renv", ".Rproj.user", ".quarto",
             "node_modules", "packrat", "venv")
